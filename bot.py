...
def show_high_scores(bot, update, query=None, user=None):
    ...
    reply_text = 'Uw top 5:\r\n%s' % "\r\n".join(['{score}: {duration} ({date})'.format(duration=r['duration'], date=str(r['date']), score=r['score']) for r in high_scores['my_top']])
    print(reply_text)
    if query:
      query.message.reply_text(reply_text)
    else:
      update.message.reply_text(reply_text)
    reply_text = 'Algemene top 5:\r\n%s' % "\r\n".join(['{user}: {score} ({duration})'.format(user=str(r['user']), score=r['score'], duration=r['duration']) for r in high_scores['global_top']])
    print(reply_text)
    if query:
      query.message.reply_text(reply_text)
    else:
